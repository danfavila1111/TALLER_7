...
import co.edu.sena.project2687365.util.ConnectionPool;
    ...
    public Integer saveObj(User user) throws SQLException {
        ...
        if (user.getUser_id()!=null && user.getUser_id()>0){
            sql = "update users_tbl set user_firstname=?, user_lastname=?,user_email=?,"+
                    "user_password= aes_encrypt(?,'$2a$12$Jea.ZfUxizSY2/HH9JO7aOVBw2.28uQBWD1C9hk7XAYTiIZx5hDLa')"
                    +" where user_id=?";

        } else {
            sql = "insert into users_tbl(user_firstname,user_lastname,user_email,user_password)"+
                    "values(upper(?),upper(?),upper(?),"+"aes_encrypt(?,'$2a$12$Jea.ZfUxizSY2/HH9JO7aOVBw2.28uQBWD1C9hk7XAYTiIZx5hDLa'))";
        }
        try(Connection conn =ConnectionPool.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql)) {
